...
<div class="mb-8">
  ...
  <h1 class="font-serif text-[26px] font-semibold text-[#101820]">Edit Job Circular</h1>
  <p class="text-[14px] text-[#6B7178] mt-1">Update job circular details for <strong><?= e($job['title']) ?></strong>.</p>
</div>
  ...
  <form method="POST" action="<?= url('/portal/jobs/' . $job['id'] . '/update') ?>" class="space-y-6">
    ...
    <div class="grid grid-cols-1 sm:grid-cols-2 gap-5">
      <div>
        ...
        <input type="text" name="title" value="<?= e($job['title']) ?>" required
      ...
      <div>
        ...
        <input type="text" name="company_name" value="<?= e($job['company_name']) ?>" required
               class="w-full px-4 py-2.5 rounded-xl border border-gray-200 text-[13.5px] focus:outline-none focus:border-[#800020]">
      </div>
    </div>
    ...
    <div class="p-5 rounded-xl border-2 border-[#800020]/20" style="background:rgba(128,0,32,0.03);">
      ...
      <div class="grid grid-cols-1 sm:grid-cols-2 gap-4 mt-3">
        <label class="p-4 rounded-xl border border-gray-200 bg-white flex items-start gap-3 cursor-pointer hover:border-[#800020] transition-colors">
          <input type="radio" name="visibility" value="members" <?= $job['visibility']==='members'?'checked':'' ?> class="mt-1 accent-[#800020]">
          <div>
        ...
        <label class="p-4 rounded-xl border border-gray-200 bg-white flex items-start gap-3 cursor-pointer hover:border-[#2F8863] transition-colors">
          <input type="radio" name="visibility" value="public" <?= $job['visibility']==='public'?'checked':'' ?> class="mt-1 accent-[#2F8863]">
          <div>
    ...
    <!-- Status Option -->
    <div>
      <label class="block text-[13px] font-semibold text-[#101820] mb-2">Job Status</label>
      <select name="status" class="w-full sm:w-1/2 px-4 py-2.5 rounded-xl border border-gray-200 text-[13.5px] bg-white focus:outline-none focus:border-[#800020]">
        <option value="active" <?= $job['status']==='active'?'selected':'' ?>>Active (Open for applications)</option>
        <option value="closed" <?= $job['status']==='closed'?'selected':'' ?>>Closed (No longer accepting applications)</option>
      </select>
    </div>
    ...
    <div class="grid grid-cols-1 sm:grid-cols-3 gap-5">
      ...
      <div>
        ...
        <input type="text" name="location" value="<?= e($job['location']) ?>"
               class="w-full px-4 py-2.5 rounded-xl border border-gray-200 text-[13.5px] focus:outline-none focus:border-[#800020]">
      </div>

      <div>
        <label class="block text-[13px] font-semibold text-[#101820] mb-2">Salary Range</label>
        <input type="text" name="salary_range" value="<?= e($job['salary_range']) ?>"
               class="w-full px-4 py-2.5 rounded-xl border border-gray-200 text-[13.5px] focus:outline-none focus:border-[#800020]">
      </div>
    </div>
    ...
    <div>
      ...
      <input type="date" name="deadline" value="<?= e($job['deadline']) ?>"
    ...
    <div>
      ...
      <textarea name="description" rows="5" required
                class="w-full px-4 py-2.5 rounded-xl border border-gray-200 text-[13.5px] focus:outline-none focus:border-[#800020]"><?= e($job['description']) ?></textarea>
    </div>

    <!-- Requirements -->
    <div>
      <label class="block text-[13px] font-semibold text-[#101820] mb-2">Requirements</label>
      <textarea name="requirements" rows="4"
                class="w-full px-4 py-2.5 rounded-xl border border-gray-200 text-[13.5px] focus:outline-none focus:border-[#800020]"><?= e($job['requirements']) ?></textarea>
    </div>

    <!-- Application Method Selection -->
    <div x-data="{ applyType: '<?= e($job['apply_type'] ?? 'portal') ?>' }" class="p-5 rounded-xl border-2 border-indigo-900/15" style="background:rgba(16,24,32,0.02);">
      ...
      <div x-show="applyType === 'external_link'" x-transition class="mt-4">
        <label class="block text-[12.5px] font-semibold text-[#101820] mb-1.5">Application Website Link (URL)</label>
        <input type="url" name="apply_link" value="<?= e($job['apply_link'] ?? '') ?>" placeholder="https://careers.company.com/apply/123"
      ...
      <div x-show="applyType === 'email'" x-transition class="mt-4">
        <label class="block text-[12.5px] font-semibold text-[#101820] mb-1.5">HR / Application Email Address</label>
        <input type="email" name="apply_email" value="<?= e($job['apply_email'] ?? '') ?>" placeholder="hr@company.com"
               class="w-full px-4 py-2.5 rounded-xl border border-gray-200 text-[13.5px] bg-white focus:outline-none focus:border-[#800020]">
      </div>
    </div>
    ...
    <div>
      ...
      <textarea name="how_to_apply" rows="3"
                class="w-full px-4 py-2.5 rounded-xl border border-gray-200 text-[13.5px] focus:outline-none focus:border-[#800020]"><?= e($job['how_to_apply']) ?></textarea>
    </div>
    ...
    <div class="pt-4 border-t border-gray-100 flex items-center justify-end gap-3">
      ...
      <button type="submit" class="px-7 py-2.5 rounded-xl text-[14px] font-semibold text-white transition-all shadow-md hover:-translate-y-0.5"
        ...
        <i class="fa-solid fa-floppy-disk mr-2"></i> Update Job Circular
      </button>
    </div>
  </form>
</div>

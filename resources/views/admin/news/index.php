  ...
  <table class="w-full text-[13px]">
    ...
    <tbody class="divide-y divide-white/5">
      ...
      <?php foreach ($news as $n):
        $cat = $n['category'] ?? 'news';
        $catBadges = [
          'news' => ['General News', 'rgba(99,102,241,0.15)', '#818cf8', '📰'],
          'press_release' => ['Press Release', 'rgba(234,179,8,0.15)', '#fde047', '📣'],
          'notice' => ['Official Notice', 'rgba(239,68,68,0.15)', '#f87171', '📌'],
          'resolution' => ['Meeting Resolution', 'rgba(78,156,129,0.15)', '#6ee7b7', '📜'],
        ];
      ...
      <tr>
        ...
        <td class="px-5 py-3.5">
          ...
          <a href="<?= asset('storage/news/' . e($n['attachment_file'])) ?>" target="_blank" class="inline-flex items-center gap-1 px-2 py-0.5 rounded text-[11px] bg-amber-950/60 text-amber-300 border border-amber-800/60 hover:text-white">
        ...
        <td class="px-5 py-3.5 space-x-2">
          <a href="<?= url('/news/' . e($n['slug'] ?: $n['id'])) ?>" target="_blank" class="text-blue-400 hover:underline">View</a>
          <a href="<?= url('/admin/news/' . $n['id'] . '/edit') ?>" class="text-[#E58E97] hover:underline">Edit</a>

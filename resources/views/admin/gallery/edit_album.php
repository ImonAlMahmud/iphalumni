<?php
/**
 * Admin Gallery Edit Album Form View
 * Variables: $album
 */
?>
<div class="mb-6">
...
<div class="p-8 rounded-3xl" style="background:rgba(255,255,255,0.04);border:1px solid rgba(255,255,255,0.08);">
  <form method="POST" action="<?= url('/admin/gallery/' . $album['id'] . '/edit') ?>" class="space-y-5">
    <?= csrf_field() ?>

    <div>
      <label class="block text-[13px] font-medium text-white/70 mb-1.5" for="title">Album Title</label>
      <input id="title" type="text" name="title" value="<?= e($album['title']) ?>" required
    ...
    <div>
      ...
      <textarea id="description" name="description" rows="3"
                class="w-full px-4 py-2.5 rounded-xl text-[14px] text-white focus:outline-none"
                style="background:rgba(255,255,255,0.06);border:1px solid rgba(255,255,255,0.12);"
                placeholder="Brief details about the album..."><?= e($album['description']) ?></textarea>
    </div>

    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
      <div>
        <label class="block text-[13px] font-medium text-white/70 mb-1.5" for="album_date">Album Date</label>
        <input id="album_date" type="date" name="album_date" value="<?= date('Y-m-d', strtotime($album['album_date'])) ?>" required
               class="w-full px-4 py-2.5 rounded-xl text-[14px] text-white focus:outline-none"
               style="background:rgba(255,255,255,0.06);border:1px solid rgba(255,255,255,0.12);">
      </div>
      <div>
    ...
    <button type="submit" class="px-6 py-2.5 rounded-xl text-[14px] font-semibold text-white"
            style="background:linear-gradient(135deg,#A22638,#800020);">Update Album</button>
  </form>
</div>

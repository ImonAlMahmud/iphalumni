...
class UddoktaPayController extends BaseController
{
    ...
    public function initiate(Request $request)
    {
        ...
        if ($membershipId > 0) {
            ...
            $existing = (new Membership())->getByAlumni((int)$profile['id']);
            if ($existing && $existing['status'] === 'active') {
                return redirect('/portal/membership')->with('info', 'You already have an active membership.');
            }

            // Reuse pending membership if exists, otherwise create new
            if ($existing && $existing['status'] === 'pending') {
                ...
                $memberNum = 'IPH-' . ($profile['batch_year'] ?? date('Y')) . '-' . str_pad((string)$profile['id'], 4, '0', STR_PAD_LEFT);
    ...
    public function webhook(Request $request)
    {
        ...
        $payload = $request->all();
        Log::info('UddoktaPay Webhook Received:', $payload);
